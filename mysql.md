...
​	MVCC是一种用来解决读写冲突的无锁并发控制，也就是为事务分配单项增长的时间戳，为每个修改保存一个版本，版本与事务时间戳关联，读操作值读该事务开始前的数据库的快照，所以MVCC可以为数据库解决以下问题：
...
4.  在经常需要根据范围进行搜索的列上创建索引，国为索引已经排序，其指定的范围是连续的
5. 在经常需要排序的列上，国为索引已经排序，这样井底可以利用索引的排序，加快排序井底时间
6. 在经常使用在 where 子句中的列上，加快条件的判断速度
6. 字符串建立索引要注意大小，索引长度过长，占用的空间也就越大，适当的可以截取进行索引，缺点是不能使用到覆盖索引，具体根据业务合理安排。
...
##### 54. innodb的事务如何通过日志实现的

预写入方式：

事务日志是通过redo和innodb的存储日志引擎日志患侧来实现的。当开始一个事务的时候，会记录事务的lsn号，当事务是指的时候，会往innodb存储引擎的日志的日志缓存里面插入事务日志；当事务提交的时候，必须将存储引擎的日志缓存写入磁盘（innodb_flush_log_at_trx_commit控制），也就是写数据前需要先写入日志。

##### 55。 如何设计或配置MySQL，才能达到高效使用的目的？

1.数据库设计方面，字段类型等

2.系统架构方面，表散列(分表)，集群，读写分离
...
##### 56. 数据库设计时，常遇到的性能瓶颈有哪些，常用的解决方案？

瓶颈有：

1.磁盘搜索，优化  将数据分布在多个磁盘
...
##### 57.超键、候选键、主键、外键分别是什么？

超键：在关系中能唯一标识元祖的属性集称为关系模式的超键。一个属性可以作为一个超键，多个属性组合在一起也可以作为一个超键，超键包含候选键和主键.

候选键：是最小超键，即没有冗余元素的超键。

主键：数据表的数据列的唯一标识，不能缺失也不能为null

外键：在一个表中存在的另一个表的主键呈词表的外键。

##### 59.什么是表分区？表分区与分表的区别？表分区有什么好处？

表分区：是指根据一定的规则，将数据库中的一张表分解成多个更小的，容易管理的部分。从逻辑上看，只有一张表，但是底层缺失右多个无力分区组成的。

分表：通过一定的规则，将一张表分解成多张不同的表

###### 分区的好处：

1.数据可以分布在不同的物理设备上，从而高效的利用多个硬件

2.和单个磁盘或者文件系统相比，可以存储更多的数据
...
##### 62、假设现在有人操作数据库，不小心执行错了语句，误删除了很多数据，这时候能恢复吗？咋么恢复。
...
首先主库还是要开启 bin-log ， 从库先设置要连接的主库 change master…… 然后执行 start slave，这时候从库会创建两个线程，一个 io_thread ，主要负责连接主数据库。一个sql_thread 主要是负责执行中转日志语句。首先，主库接收到从库的同步请求，根据传递的 bin-log 文件名和开始同步的位置，发送二进制文件给从库，从库 io_thread 负责把接收到的数据放入到中转日志，然后 sql_thread 负责从中转日志读取解析执行，执行完成，更新同步的位置标志。

##### 64、你知道主从延迟吗？有些时候延迟的时间还会很长。遇到这种情况咋么办？
...
##### 65. MySQL 主从复制是如何实现的？如果从节点挂了，重新启动从节点，如何保持数据一致性？如何解决数据同步延迟问题？

##### 66. 说说你优化 SQL 语句的几种思路有哪些？

##### 67. 事务中隔离性，有几种情况？这几种情况分别会产生什么问题？

##### 68. Slow Log（有什么用，什么时候需要）

##### 69. 常用 MySQL 函数，如：now ()、md5 ()、concat ()、uuid () 等

##### 70. mysql把一个大表拆分多个表后,如何解决跨表查询效率问题
